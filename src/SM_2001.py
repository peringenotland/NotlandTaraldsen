...
L_0 = 0 # Loss-carryforward in millions
X_0 = 556 # Initial cash balance in millions
PPE_0 = 125.2 # Initial property, plant, and equipment in millions
shares_outstanding = 269.311 # Shares outstanding in millions
employee_stock_options = 26.249 # Employee stock options outstanding in millions
D = 0.33 # Depreciation as a fraction of PPE
CE = 0.13 # Capital expenditures as a fraction of Revenues
mu_0 = 0.434  # Initial growth rate per year
sigma_0 = 0.167  # Initial revenue volatility per year
eta_0 = 0.05 # Initial volatility of expected growth rate # TODO: Check this value
gamma_0 = 0.71 # Initial variable cost as a fraction of revenues
phi_0 = 0.06 # Initial volatility of variable cost
phi_mean = 0.03 # Mean-reversion level for variable cost volatility
rho_R_mu = 0.0 # Correlation between revenue and growth rate
rho_R_gamma = 0.0 # Correlation between revenue and variable cost
rho_mu_gamma = 0.0 # Correlation between growth rate and variable cost
mu_mean = 0.05  # Mean-reversion level for growth rate
sigma_mean = 0.083  # Mean-reversion level for volatility
gamma_mean = 0.71  # Mean-reversion level for variable cost
taxrate = 0.35  # Corporate tax rate
r_f = 0.05  # Risk-free rate
kappa_sigma = np.log(2) / 2.8  # Mean-reversion speed for volatility, half-life 2.8 years
kappa_mu = kappa_sigma  # Mean-reversion speed for growth rate
kappa_eta = kappa_sigma  # Mean-reversion speed for expected growth rate volatility
kappa_gamma = kappa_sigma  # Mean-reversion speed for growth rate volatility
kappa_phi = kappa_sigma  # Mean-reversion speed for variable cost volatility
F = 43 # Fixed costs in millions per year
estimated_beta = 2.67 # Estimated beta from a regression
bloomberg_beta = 1.76 # Bloomberg beta
market_risk_premium = 0.06 
lambda_mean = bloomberg_beta * (market_risk_premium / sigma_0)
# TODO: Check the value of lambda_mean
...
# Simulation setup
num_steps = T  # Simulations on annual basis
revenues = np.zeros((simulations, num_steps))
# costs = np.zeros((simulations, num_steps))
ppe_values = np.zeros((simulations, num_steps))
...
phi_values = np.zeros((simulations, num_steps))
gamma_values = np.zeros((simulations, num_steps))
...
for t in range(1, num_steps):
    ...
    dW3 = np.random.normal(0, np.sqrt(dt), simulations) # Brownian motion for variable cost volatility
    ...
    mu_values[:, t] = np.exp(-kappa_mu * dt) * mu_values[:, t-1] + (1 - np.exp(-kappa_mu * dt)) * (mu_mean) + (np.sqrt((1 - np.exp(-2 * kappa_mu * dt)) / (2 * kappa_mu)) * eta_values[:, t-1] * dW2)
    ...
    phi_values[:, t] = phi_0 * np.exp(-kappa_phi * (t)) + phi_mean * (1 - np.exp(-kappa_phi * (t)))
    
    # Update revenue using stochastic process
    revenues[:, t] = revenues[:, t-1] * np.exp((mu_values[:, t-1] - lambda_mean*sigma_values[:, t-1] - (0.5 * sigma_values[:, t-1]**2)) * dt + sigma_values[:, t-1] * dW1)

    gamma_values[:, t] = np.exp(-kappa_gamma * dt) * gamma_values[:, t-1] + (1 - np.exp(-kappa_gamma * dt)) * gamma_mean + (np.sqrt((1 - np.exp(-2 * kappa_gamma * dt)) / (2 * kappa_gamma)) * phi_values[:, t-1] * dW3)
    
    # Compute costs and expenses
    cost = gamma_values[:, t] * revenues[:, t] + F
    Dep = D * ppe_values[:, t-1]
    CapEx = CE * revenues[:, t-1]
    ppe_values[:, t] = ppe_values[:, t-1] - Dep + CapEx

    EBIT = revenues[:, t] - cost - Dep
    ...
    cash_balances[:, t] = r_f * cash_balances[:, t-1] + EBIT + Dep - taxes - CapEx
...
# Plot a few ppe trajectories
plt.figure(figsize=(10, 6))
for i in range(10000):
    plt.plot(ppe_values[i, :], alpha=0.3)
plt.xlabel("Quarters")
plt.ylabel("ppe (millions)")
plt.title("Monte Carlo Simulation of ppe Over Time")
plt.show()
...
# revenue_quantiles = np.quantile(revenues[:, [1, 3, 5, 7, 10]], quantiles, axis=0)  # Extract values for 1, 3, 5, 7, 10 years forward
...
# revenue_table.loc["Mean"] = np.mean(revenues[:, [1, 3, 5, 7, 10]], axis=0)
