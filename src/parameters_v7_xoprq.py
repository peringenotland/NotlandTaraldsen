...
def plot_total_costs_from_data(df=FUNDAMENTALS):
    """
    Computes and plots the total costs (cogsq + xsgaq + xoproq) over time for each firm identified by gvkey.
    Missing values are treated as zero. Assumes 'datadate' is a datetime or sortable time column.
    """
    # Ensure datadate is in datetime format
    df['datadate'] = pd.to_datetime(df['datadate'])

    # Compute total cost
    df['total_cost'] = df['cogsq'].fillna(0) + df['xsgaq'].fillna(0) + df['xoproq'].fillna(0)

    # Group by gvkey and datadate
    grouped = df.groupby(['gvkey', 'datadate'])['total_cost'].sum().reset_index()

    # Pivot to wide format for plotting
    pivot_df = grouped.pivot(index='datadate', columns='gvkey', values='total_cost')

    # Plot
    plt.figure(figsize=(12, 6))
    pivot_df.plot()
    plt.title("Total Costs Over Time by Firm (gvkey)")
    plt.xlabel("Date")
    plt.ylabel("Total Cost")
    plt.legend(title='gvkey', bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.grid(True)
    plt.show()
...
if __name__ == '__main__':
    ...
    plot_total_costs_from_data(df=FUNDAMENTALS)
